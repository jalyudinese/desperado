...
return comments;};var paginator=function(callback){if(hasMore()){var url=config.feed+'?alt=json&v=2&orderby=published&reverse=false&max-results=500';if(cursor){url+='&published-min='+new Date(cursor).toISOString();}
...
return'';};var replybox=null;var replyUrlParts=null;var replyParent=undefined;var onReply=function(commentId,domId){if(replybox==null){replybox=document.getElementById('comment-editor');if(replybox!=null){replybox.height='430px';replybox.style.display='block';replyUrlParts=replybox.src.split('#');}}
